...
import { TestimonialInfo } from "@/types/api/testimonial.type";
import { PageProps, SEOData } from "@/types/index.type";
import { Metadata } from "next";

export async function generateMetadata({ params }: any): Promise<Metadata> {
	const slug = (await params)?.slug;
	const data: SEOData = await http(`/seo/testimonial/${slug}`);
	return getMetadata(data);
}

const Page = async ({ params }: PageProps<{ slug: string }>) => {
	const slug = (await params)?.slug;
	const data: TestimonialInfo = await http(`/page/testimonial/by-slug/${slug}`);
	return (
		<>
			<InnerBanner
