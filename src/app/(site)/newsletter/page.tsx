...
import { NewsletterPageInfo } from "@/types/api/newsletter.type";
...
export async function generateMetadata(): Promise<Metadata> {
	const data: SEOData = await http(`/seo/page/newsletter`);
	return getMetadata(data);
}
const Page = async ({ searchParams }: PageProps<{ vertical: string }>) => {
	const vertical = (await searchParams)?.vertical;
	const url = vertical
		? `/page/newsletter?vertical=${vertical}`
		: `/page/newsletter`;
	const data: NewsletterPageInfo = await http(url);
			...
			<section className="m-section">
				<div className="relative p-section ">
					...
					<div className="mani-title-wrapper flex items-center justify-between">
						<div className="">
							<h2 className="mani-title text-left">NEWSLETTERS</h2>
